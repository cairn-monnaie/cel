...
PATH_ACCESS_MANAGEMENT=tests/performances/AccessManagement
...
jmeter -n -l $PATH_LOGIN/$date.$hour.csv -t $PATH_LOGIN/load_test_by_steps.jmx 
echo "$(tput setaf 2) Load testing is over ! "

echo "$(tput setaf 3) Start load test for access management$(tput sgr 0)"
jmeter -n -l $PATH_ACCESS_MANAGEMENT/$date.$hour.csv -t $PATH_ACCESS_MANAGEMENT/load_test_by_steps.jmx 
echo "$(tput setaf 2) Load testing is over ! "
